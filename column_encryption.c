...
bytea*      pg_column_encrypt(bytea* input_data);
Datum       pg_column_decrypt(key_detail* entry, bytea* encrypted_data);
bytea*      add_header_to_encrypted_data(bytea* encrypted_data);
...
bool         is_session_opened(void);

PG_FUNCTION_INFO_V1(pg_stat_actv_mask);
PG_FUNCTION_INFO_V1(column_enc_text_in);
PG_FUNCTION_INFO_V1(column_enc_text_out);
PG_FUNCTION_INFO_V1(column_enc_bytea_in);
PG_FUNCTION_INFO_V1(column_enc_bytea_out);
PG_FUNCTION_INFO_V1(column_enc_comp_eq_text);
PG_FUNCTION_INFO_V1(column_enc_comp_eq_bytea);
...
PG_FUNCTION_INFO_V1(column_enc_text_regclass);
PG_FUNCTION_INFO_V1(enc_hash_encrted_data);
PG_FUNCTION_INFO_V1(enc_store_key_detail);
PG_FUNCTION_INFO_V1(enc_store_previous_key_detail);
PG_FUNCTION_INFO_V1(enc_remove_key_detail);
PG_FUNCTION_INFO_V1(enc_drop_previous_key_detail);
PG_FUNCTION_INFO_V1(enc_rename_backupfile);
PG_FUNCTION_INFO_V1(column_enc_recv);
PG_FUNCTION_INFO_V1(column_enc_send);
...
static char *encrypt_backup_key = "";
...
void _PG_init(void)
{
    ...
    DefineCustomBoolVariable("column_encrypt.mask_key_log",
    ...
    DefineCustomBoolVariable("column_encrypt.encrypt_enable",
    ...
    DefineCustomStringVariable("column_encrypt.backup_dir",
 ...
 * @return    nothing
 */
static void suppress_keylog_hook(ErrorData *edata){
    /*
...
Datum pg_stat_actv_mask(PG_FUNCTION_ARGS)
{
...
Datum column_enc_text_in(PG_FUNCTION_ARGS)
{
    ...
    if (encrypt_enable) {
        ...
        encrypted_data = pg_column_encrypt(plain_data);
        pfree(plain_data);

        /* add header(dummy) to encrypted data */
        result = add_header_to_encrypted_data(encrypted_data);
...
Datum column_enc_text_out(PG_FUNCTION_ARGS)
{
    ...
    if (encrypt_enable) {
        ...
        tmp_result = pg_column_decrypt(entry, encrypted_data);
...
Datum column_enc_bytea_in(PG_FUNCTION_ARGS)
{
    char     *input_text = PG_GETARG_CSTRING(0);  /* input plain text parameter */
    bytea    *encrypted_data = NULL; /* encryption data */
...
Datum column_enc_bytea_out(PG_FUNCTION_ARGS)
{
    bytea *input_data = PG_GETARG_BYTEA_PP(0); /* pointer of input ciphertext  */
...
/*
 * Function : column_enc_comp_eq_text
 * ---------------------
...
Datum column_enc_comp_eq_text(PG_FUNCTION_ARGS)
{
...
/*
 * Function : column_enc_comp_eq_bytea
 * ---------------------
...
Datum column_enc_comp_eq_bytea(PG_FUNCTION_ARGS)
{
    bytea *barg1  = PG_GETARG_BYTEA_PP(0);
    bytea *barg2  = PG_GETARG_BYTEA_PP(1);
    bool result = binary_comparison(barg1, barg2);
...
Datum bool_enc_text(PG_FUNCTION_ARGS)
{
    ...
    PG_RETURN_DATUM(DirectFunctionCall1(column_enc_text_in, CStringGetDatum(str)));
}


Datum enc_text_trim(PG_FUNCTION_ARGS)
{
    text   *str = (text *)DatumGetPointer(DirectFunctionCall1(rtrim1, PG_GETARG_DATUM(0)));

    PG_RETURN_DATUM(DirectFunctionCall1(column_enc_text_in, CStringGetDatum(text_to_cstring(str))));
}


Datum inet_enc_text(PG_FUNCTION_ARGS)
{
...
Datum xml_enc_text(PG_FUNCTION_ARGS)
{
...
Datum column_enc_text_regclass(PG_FUNCTION_ARGS)
{
    char *str = NULL;

    str = (char *)DatumGetCString(DirectFunctionCall1(column_enc_text_out, PG_GETARG_DATUM(0)));
...
Datum enc_hash_encrted_data(PG_FUNCTION_ARGS)
{
...
/*
 * Function : enc_store_key_detail
 * ---------------------
 * regist current_key_detail
 *
 * @param    *text ARG[0]    encryption key
 * @param    *text ARG[1]    encryption algorithm
 */

Datum enc_store_key_detail(PG_FUNCTION_ARGS)
{
...
/*
 * Function : enc_store_previous_key_detail
 * ---------------------
...
Datum enc_store_previous_key_detail(PG_FUNCTION_ARGS)
{
    text *key = PG_GETARG_TEXT_P(0); /* encryption key */
    text *algorithm = PG_GETARG_TEXT_P(1); /* encryption algorithm */
...
/* Function : enc_remove_key_detail
 * ---------------------
 * drop cipher key information from memory
 */

Datum enc_remove_key_detail(PG_FUNCTION_ARGS)
{
...
/*
 * Function : enc_drop_previous_key_detail
 * ---------------------
...
Datum enc_drop_previous_key_detail(PG_FUNCTION_ARGS)
{
...
Datum enc_rename_backupfile(PG_FUNCTION_ARGS)
{
...
/*
 *        copy from PostgreSQL 10.0(backend/utils/adt/varlena.c)
 *        bytearecv            - converts external binary format to bytea
 */
Datum column_enc_recv(PG_FUNCTION_ARGS)
{
...
/*
  *              column_enc_send                       - converts bytea to binary format
  *
  * This is a special case: just copy the input...
  */
Datum column_enc_send(PG_FUNCTION_ARGS)
{
